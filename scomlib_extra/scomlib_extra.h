...
#define SCOM_DATALOG_TRANSFER_OBJECT_TYPE ((scom_object_type_t)0x0101)
    ...
    typedef struct
    {
        ...
        /** \brief pointer with the begining of the encoded data which needs to be send out; only valid when no error is set */
        char *data;
    ...
    typedef struct
    {
        ...
        /** \brief pointer with the begining of the decoded property data; only valid when no error is set */
        char *data;
...
#define SCOMX_DEST_GATEWAY ((scomx_dest_t)(1))
    ...
    // NOTE: These objects are nicely documented in the "Technical specification - Xtender serial protocol appendix" PDF.
    // That file is inside "Communication protocols Xcom-232i" zip file downloadable from "Software and Updates"
    // sub-section of "Download center" at https://www.studer-innotec.com/
    ...
    typedef enum
    {
        ...
        SCOMX_INFO_XTENDER_BATT_CYCLE_PHASE = 3010, //# enum: 0-invalid, 1-bulk, 2-absorb, 3-equalise, 4-floating, 5-r.float., 6-per.abs., 7-missing, 8-forming
        ...
        SCOMX_INFO_XTENDER_AUX1_MODE = 3054,        // enum: 0-invalid, 1-A(automatic), 2-I(inv.automatic), 3-M(manual?), 4-M, 5-G(generator)
        SCOMX_INFO_XTENDER_AUX2_MODE = 3055,        // enum: 0-invalid, 1-A, 2-I, 3-M, 4-M, 5-G
        SCOMX_INFO_XTENDER_LOCKING_FLAGS = 3056,    // float bits: 0-forbidden inverter, 1-forbidden charger, 2-forbidden boost, 3-forbidden transfer, 4-forbidden injection, 8-mode integral, 9 f. multi, 10-mult independants, 11-standy slave allowed
        ...
        */
        ...
        SCOMX_INFO_VARIOTRACK_OPER_MODE = 11016,   // enum: 0-night, 1-startUp, 2-, 3-charger, 4-, 5-security, 6-OFF, 7-, 8-charge, 9-chargeV, 10-chargeI, 11-chargeT
        ...
        SCOMX_INFO_VARIOTRACK_ERROR_TYPE = 11034,  // enum: 0-no error, 1-BatOverV, 2-Earth, 3-NoBatt, 4-OverTemp, 5-BatOverV, 6-PvOverV, 7-others, 8-11--
        SCOMX_INFO_VARIOTRACK_DAYS_EQUAL = 11037,  // float, days until next equalization
        SCOMX_INFO_VARIOTRACK_BAT_CYCLE = 11038,   //# cycle phase, enum: 0-bulk, 1-absorb., 2-equalize, 3-floating, 4-, 5-, 6-r.float., 7-per.abs.
        ...
        SCOMX_INFO_VARIOTRACK_AUX1_MODE = 11063,  // enum: 0-invalid, 1-A(automatic), 2-I(inv.automatic), 3-M(manual?), 4-M, 5-G(generator)
        SCOMX_INFO_VARIOTRACK_AUX2_MODE = 11064,  // enum: 0-invalid, 1-A(automatic), 2-I(inv.automatic), 3-M(manual?), 4-M, 5-G(generator)
    ...
    typedef enum
    {
        ...
        // "parameter" object "value_qsp" that can be entered on the remote control in level qsp or installer (one of the QSP_...)
        SCOMX_PROP_PARAMETER_VALUE_QSP = 0x5,
        // "parameter" object "min_qsp" - Minimum that canbeentered on the remote control in level qsp or installer
        SCOMX_PROP_PARAMETER_MIN_QSP = 0x6,
        // "parameter" object "max_qsp" - Maximum that can be entered on the remote control in level qsp or installe
        SCOMX_PROP_PARAMETER_MAX_QSP = 0x7,
        // "parameter" object "level_qsp" - accessibility level of this parameter modifiable in level qsp or install
        // enum 0x00-VIEW_ONLY, 0x10-BASIC, 0x20-EXPERT, 0x30-INSTALLER, 0x40-QSP
        SCOMX_PROP_PARAMETER_LEVEL_QSP = 0x8,
        // "parameter" object "unsaved_value_qsp" - the value that can be entered on the remote control in level qsp or installer, but without saving the value in internal flas
    ...
    // Decode frame header from data read from the port. Read data must be exactly SCOM_FRAME_HEADER_SIZE bytes long.
    ...
    // Reads native-endian uint32_t or uint16_t value from the response if the response is valid and long enough
    uint32_t scomx_result_int(scomx_dec_result_t res);
    // Reads native float value from the response if the response is valid and long enough. May not work on all platforms.
